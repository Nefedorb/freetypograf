...
fn install_tray(app: &mut App) -> tauri::Result<()> {
  let show_settings = MenuItem::with_id(app, "show_settings", "Открыть настройки", true, None::<&str>)?;
  let toggle_button = MenuItem::with_id(app, "toggle_button", "Показать/скрыть кнопку", true, None::<&str>)?;
...
fn send_copy_or_paste(key: char) -> Result<(), String> {
  ...
  if status.success() {
    ...
    Err("macOS не отправила shortcut. Проверьте Accessibility-разрешение для приложения.".to_string())
  }
}
